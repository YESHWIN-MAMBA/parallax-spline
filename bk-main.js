...
// Lights: softer & less punchy (closer to screenshot)
...
// Ground plane for soft fade feel
...
/**
 * Improved spacing/composition:
 * - fewer overlaps
 * - more even distribution around the headline
 * - depth variation similar to screenshot
 */
function populate() {
  clearObjects();
  ...
  bigLeft.rotation.set(0.08, 0.15, 0.0);
  ...
  const midYellow = roundedBlock({ size: 1.25, radius: 0.42, color: 0xffd400 });
  midYellow.position.set(4.6, 1.05, 0.95);
  midYellow.rotation.set(-0.1, 0.35, 0.12);

  const midPink = roundedBlock({ size: 1.05, radius: 0.38, color: 0xff2bd6 });
  midPink.position.set(3.1, 0.05, 1.45);
  midPink.rotation.set(0.18, 0.18, -0.22);
  ...
  centerGreenSmall.rotation.set(0.05, 0.2, 0.1);

  // extra floaters (spaced wider)
  const extras = [];
  const extraCount = 12;
  for (let i = 0; i < extraCount; i++) {
    ...
    // spread: wider X, controlled Y, varied Z
    e.position.set(
      Math.random() * 14 - 7,
      Math.random() * 5.8 - 2.9,
      Math.random() * -8 + 1.2,
...
function setHovered(obj) {
  ...
  if (hovered) {
    ...
    hovered.material.emissiveIntensity = 0.06; // subtle glow
  }
...
function animate() {
  ...
  // scroll depth (slightly stronger)
